...
const postgres = !postgres_1.postgres ? null : postgres_1.postgres;
class VMManager {
    constructor({ isLarge, region, limitSize, minSize, hostname }) {
        this.isLarge = false;
        this.region = 'US';
        this.limitSize = 0;
        this.minSize = 0;
        ...
        this.getCurrentSize = async () => {
            const { rows } = await postgres.query(`SELECT count(1) FROM vbrowser WHERE pool = $1`, [this.getPoolName()]);
        ...
        this.getAvailableCount = async () => {
            const { rows } = await postgres.query(`SELECT count(1) FROM vbrowser WHERE pool = $1 and state = 'available'`, [this.getPoolName()]);
            return Number(rows[0]?.count);
        };
        this.getStagingCount = async () => {
            const { rows } = await postgres.query(`SELECT count(1) FROM vbrowser WHERE pool = $1 and state = 'staging'`, [this.getPoolName()]);
            return Number(rows[0]?.count);
        };
        this.getAvailableVBrowsers = async () => {
            const { rows } = await postgres.query(`SELECT vmid from vbrowser WHERE pool = $1 and state = 'available'`, [this.getPoolName()]);
            return rows.map((row) => row.vmid);
        };
        this.getStagingVBrowsers = async () => {
            const { rows } = await postgres.query(`SELECT vmid from vbrowser WHERE pool = $1 and state = 'staging'`, [this.getPoolName()]);
            return rows.map((row) => row.vmid);
        };
        this.getTag = () => {
        ...
        this.assignVM = async (roomId, uid) => {
            if (!roomId || !uid) {
                return undefined;
            }
    ...
    RETURNING data, pass`, [roomId, uid, this.getPoolName()]);
            let vm = rows[0]?.data;
            let pass = rows[0]?.pass;
            if (!vm) {
                return;
            }
            return { ...vm, pass, assignTime: Number(new Date()) };
        };
        this.resetVM = async (vmid, roomId) => {
            if (roomId !== undefined) {
                // verify the roomId matches if user initiated
                const { rows } = await postgres.query(`SELECT "roomId" FROM vbrowser WHERE pool = $1 AND vmid = $2`, [this.getPoolName(), vmid]);
                if (rows[0]?.roomId && rows[0]?.roomId !== roomId) {
                    console.log('[RESET] %s: roomId mismatch on %s, expected %s, got %s', this.getPoolName(), vmid, rows[0]?.roomId, roomId);
                    return;
                }
            }
            ...
            if (this.reuseVMs) {
                ...
                const { rows } = await postgres.query(`SELECT image FROM vbrowser WHERE pool = $1 AND vmid = $2`, [this.getPoolName(), vmid]);
                ...
                if (this.imageId !== vmImageId) {
                    ...
                    await postgres.query(`UPDATE vbrowser SET image = $3 WHERE pool = $1 AND vmid = $2`, [this.getPoolName(), vmid, this.imageId]);
                }
                else {
    ...
    INSERT INTO vbrowser(pool, vmid, "creationTime", state) 
    VALUES($1, $2, NOW(), 'staging')`, [this.getPoolName(), id]);
            (0, redis_1.redisCount)('vBrowserLaunches');
            return id;
        };
        this.terminateVMWrapper = async (vmid) => {
            ...
            const { command, rowCount } = await postgres.query(`DELETE FROM vbrowser WHERE pool = $1 AND vmid = $2 RETURNING id`, [this.getPoolName(), vmid]);
        ...
        this.runBackgroundJobs = async () => {
            const resizeVMGroupIncr = async () => {
                ...
                if (launch) {
                    console.log('[RESIZE-INCR]', this.getPoolName(), 'target:', this.getAdjustedBuffer(), 'available:', availableCount, 'staging:', stagingCount, 'currentSize:', currentSize, 'limit:', this.getLimitSize());
                    try {
                        await this.startVMWrapper();
                    }
                    catch (e) {
                        console.log(e.response?.status, JSON.stringify(e.response?.data), e.config?.url, e.config?.data);
                    }
                }
            };
          ...
          WHERE id = (
            ...
            LIMIT 1
                        ...
                        this.getMinSize(),
                        modulo,
                        config_1.default.VM_MIN_UPTIME_MINUTES * 60, // to seconds
                    ]);
                    const first = rows[0];
                    if (first) {
            ...
            const cleanupVMGroup = async () => {
                ...
                try {
                    ...
                    console.log('[CLEANUP] %s: failed to fetch VM list', this.getPoolName());
                    return;
                }
                const { rows } = await postgres.query(`
        ...
        OR state = 'available')
        `, [this.getPoolName()]);
                const inUse = new Set(rows.map((row) => row.vmid));
                console.log('[CLEANUP] %s: found %s VMs, %s to keep', this.getPoolName(), allVMs.length, inUse.size);
        ...
        `, [this.getPoolName()]);
                const stagingPromises = rows.map(async (row) => {
                    const rowid = row.id;
                    const vmid = row.vmid;
                    const retryCount = row.retries;
                    let vm = row.data;
                    if (retryCount < this.minRetries) {
                        if (config_1.default.NODE_ENV === 'development') {
                            console.log('[CHECKSTAGING] %s: [vmid: %s] [attempt: %s] waiting for minRetries', this.getPoolName(), vmid, retryCount);
                        }
                        // Do a minimum # of retries to give reboot time
                        return [vmid, retryCount, false].join(',');
                    }
                    if (retryCount % 150 === 0) {
                        console.log('[CHECKSTAGING] %s: %s poweron, attach to network', this.getPoolName(), vmid);
                        this.powerOn(vmid);
                    ...
                    if (retryCount % 180 === 0) {
                        ...
                        await redis_1.redis?.lpush('vBrowserStageFails', vmid);
                        await redis_1.redis?.ltrim('vBrowserStageFails', 0, 24);
                    ...
                    const shouldFetchVM = retryCount === this.minRetries + 1 || retryCount % 20 === 0;
                    if (!vm && shouldFetchVM) {
                        try {
                        ...
                        if (vm?.host) {
                            // Save the VM data
                            await postgres.query(`UPDATE vbrowser SET data = $1 WHERE id = $2`, [vm, rowid]);
                        }
                    }
                    if (!vm?.host) {
                        console.log('[CHECKSTAGING] %s: no host for vm %s', this.getPoolName(), vmid);
                        throw new Error('no host for vm ' + vmid);
                    }
                    const ready = await this.checkVMReady(vm.host);
                    if (ready ||
                        retryCount % (config_1.default.NODE_ENV === 'development' ? 1 : 30) === 0) {
                        console.log('[CHECKSTAGING] %s: [ready: %s] [vmid: %s] [retries: %s] [host: %s]', this.getPoolName(), ready, vmid, retryCount, vm?.host);
                    }
                    if (ready) {
                        const passUrl = 'https://' +
                            (vm.host.includes('/')
                            ...
                            (vm.host.includes('?') ? '&' : '?') +
                            'key=' +
                            config_1.default.VBROWSER_ADMIN_KEY;
                        // console.log(passUrl);
                        const resp2 = await (0, axios_1.default)({
                            method: 'GET',
                            url: passUrl,
                            timeout: 1000,
                        });
                        ...
                        const { rows } = await postgres.query(`UPDATE vbrowser SET state = 'available', pass = $2 WHERE id = $1 AND pass IS DISTINCT FROM $2`, [rowid, pass]);
                        // console.log(rows);
                        await redis_1.redis?.lpush('vBrowserStageRetries', retryCount);
                        await redis_1.redis?.ltrim('vBrowserStageRetries', 0, 24);
                    }
            ...
            setInterval(async () => {
                console.log('[STATS] %s: currentSize %s, available %s, staging %s, target %s', this.getPoolName(), await this.getCurrentSize(), await this.getAvailableCount(), await this.getStagingCount(), this.getAdjustedBuffer());
            }, 10000);
            ...
            setImmediate(async () => {
                while (true) {
                    ...
                    try {
                        ...
                        console.warn('[CLEANUPVMGROUP-ERROR]', this.getPoolName(), e.response?.data);
                    }
